...
torch.save(generator.state_dict(), root_model+"generator_param_672.pkl")
torch.save(discriminator.state_dict(), root_model+"discriminator_param_672.pkl")
with open(root_model+"train_hist_672.pkl", 'wb') as f:
    pickle.dump(train_hist, f)

show_train_hist(train_hist, save=True, path=root+'train_hist_672.eps')

# Sample noise as generator input
z = Tensor(np.random.normal(0, 1, (672, opt.latent_dim)))
gen_imgs = generator(z)
gen_imgs = pd.DataFrame(gen_imgs.detach().numpy())  #NEW
gen_imgs.to_csv(root_data+'normal_672.csv', index=False, header=False)