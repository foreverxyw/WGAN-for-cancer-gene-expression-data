...
parser.add_argument('--feature_size', type=int, default=1385, help='size of each image dimension')
...
root = 'results/gene_data_normal_0324/'
...
torch.save(generator.state_dict(), root_model+"generator_param_88.pkl")
torch.save(discriminator.state_dict(), root_model+"discriminator_param_88.pkl")
with open(root_model+"train_hist_88.pkl", 'wb') as f:
    pickle.dump(train_hist, f)

show_train_hist(train_hist, save=True, path=root+'train_hist_88.eps')

# Sample noise as generator input
z = Tensor(np.random.normal(0, 1, (88, opt.latent_dim)))
gen_imgs = generator(z)
gen_imgs = pd.DataFrame(gen_imgs.detach().numpy())  #NEW
gen_imgs.to_csv(root_data+'normal_88.csv', index=False, header=False)

end = time.time()
print ("WGAN运行时间: %.4f秒"%(end-start))